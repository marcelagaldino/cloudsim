...
public class VmSchedulerTimeShared extends VmScheduler {
	 ...
	 */
	public VmSchedulerTimeShared(List<? extends Pe> pelist, int i) {
		super(pelist, i);
	 ...
	 */
	@Override
	public boolean allocatePesForVmInstantiation(Vm vm, List<Double> mipsShareRequested) {
		/**
		 * TODO: add the same to RAM and BW provisioners
		 */
		if (vm.isInMigration()) {
		...
		boolean result = allocatePesForVmInstantiation(vm.getUid(), mipsShareRequested);
		updatePeProvisioning();
		return result;
	}
	...
	protected boolean allocatePesForVmInstantiation(String vmUid, List<Double> mipsShareRequested) {
		double totalRequestedMips = 0;
		double peMips = getPeCapacity();
		for (Double mips : mipsShareRequested) {
			// each virtual PE of a VM must require not more than the capacity of a physical PE
