...
public class VmSchedulerTimeSharedOverSubscription extends VmSchedulerTimeShared {
	 ...
	 */
	public VmSchedulerTimeSharedOverSubscription(List<? extends Pe> pelist, int i) {
		super(pelist, i);
	}
	...
	protected boolean allocatePesForVmInstantiation(String vmUid, List<Double> mipsShareRequested) {
		...
		if (getAvailableMipsInstantiation() >= totalRequestedMips) {
			List<Double> mipsShareAllocated = new ArrayList<Double>();
			for (Double mipsRequested : mipsShareRequestedCapped) {
				if (getVmsMigratingOut().contains(vmUid)) {
					// performance degradation due to migration = 10% MIPS
			...
			getMipsMap().put(vmUid, mipsShareAllocated);
			setAvailableMipsInstantiation(getAvailableMipsInstantiation() - totalRequestedMips);
		} else {
			redistributeMipsDueToOverSubscription();
		}
