...
public class Host {
	...
	/** Sets the host's initial status - on or off  (add by me) */
	...
	public double calculateAvailableMipsInstantiation(Vm vmCandidate) {
		double totalAllocatedMipsVMs = 0;
		
		for (Vm vm : getVmList()) {
			if (vm.getId() != vmCandidate.getId()){	
		...
		double hostTotalMips =  getVmScheduler().getHostTotalMips();
		double availableMipsInstantiation = hostTotalMips - totalAllocatedMipsVMs;
		this.getVmScheduler().setAvailableMipsInstantiation(availableMipsInstantiation);
	...
	public boolean isSuitableForVm(Vm vm) {
		...
		//double currentrequestmaxmips = vm.getCurrentRequestedMaxMips();
		double availableMipsInstantiation = calculateAvailableMipsInstantiation(vm);
		//double availableMipsInstantiation2 = getVmScheduler().getAvailableMipsInstantiation();
		...
		/*return (getVmScheduler().getPeCapacity() >= vm.getCurrentRequestedMaxMips()
				&& getVmScheduler().getAvailableMipsInstantiation() >= vm.getCurrentRequestedTotalMips()
				&& getRamProvisioner().isSuitableForVm(vm, vm.getCurrentRequestedRam()) && getBwProvisioner()
				.isSuitableForVm(vm, vm.getCurrentRequestedBw()));*/
	}
	...
	public boolean vmCreate(Vm vm) {
		...
		if (!getVmScheduler().allocatePesForVmInstantiation(vm, vm.getCurrentRequestedMips())) {
			Log.printLine("[VmScheduler.vmCreate] Allocation of VM #" + vm.getId() + " to Host #" + getId()
					+ " failed by MIPS");
			getRamProvisioner().deallocateRamForVm(vm);
	 ...
	 * @post $none
	 */
	public boolean vmCreateOv(Vm vm) {
		if (getStorage() < vm.getSize()) {
			Log.printLine("[VmScheduler.vmCreate] Allocation of VM #" + vm.getId() + " to Host #" + getId()
					+ " failed by storage");
			return false;
