...
public abstract class PowerVmAllocationPolicyMigrationAbstract extends PowerVmAllocationPolicyAbstract {
	...
	public PowerHost selectHost(Map<PowerHost, Integer> nodeScoreMap) {
		...
		if (selectedHost != null) {
			...
			//caso haja mais de um host com a mesma pontuação máxima, 
			//a escolha entre eles se dá de maneira aleatória
	...
	public List<PowerHost> findHostsThatPassFilters(Vm vm, List<PowerHost> Hosts,  Set<? extends Host> excludedHosts) {
		...
		//caso não hajam filtros, apenas o filtro de recursos é executado
		if (!hasFilters()) {
		...
		for (PowerHost host : Hosts) {
			...
			if (nodeResourcesFitFilter.filtering(vm, host)) {
				...
				//é necessário verificar se o host não tem nenhuma VM, 
				//para casos em que não haja nenhum host que atenda as condições do filtro
				//ser possível alocar a nova vm em um host que não tenha nenhuma vm alocada
	...
	// executa os scores, atribuindo uma pontuação a cada nó que foi retornado da etapa de filtragem
	public Map<PowerHost, Integer> prioritizeNodes(Vm vm, List<PowerHost> filteredHosts){
		...
		/*No caso de nenhum score ser utilizado ou caso a etapa de filtragem
		 retorne apenas um nó, a pontuação atribuída será 1*/	
