...
public class PowerDatacenter extends Datacenter {
	...
	protected double updateCloudetProcessingWithoutSchedulingFutureEventsForce() {
		...
		try {
			arq1 = new FileWriter("VmsAndHosts_cs_teste3.csv", true);
			arq2 = new FileWriter("ActivesHostsAndVms.csv", true);
		} catch (IOException e) {
...
*/
		...
		for (PowerHost host : this.<PowerHost> getHostList()) {
			...
			/*for (Vm vm : host.getVmList()) {
				...
				gravarArq1.printf(vm.getId()+","+host.getId()+","+currentTime+"%n");
			}*/
		...
		//gravarArq2.printf(vms+","+hosts+","+currentTime +"%n");
			...
			for (PowerHost host : this.<PowerHost> getHostList()) {
				...
				if(host.getVmList().size()!=0) {
					for (Vm vm : host.getVmList()) {
							...
							if(vm.getCloudlet().getStatus()!=4) {
								timeFrameHostEnergy = host.getEnergyLinearInterpolation(
										previousUtilizationOfCpu,
										utilizationOfCpu,
										timeDiff,
										currentTime); //currentTime add by me
	 ...
	 * @return the double
	 */
	protected double updateCloudetProcessingWithoutSchedulingFutureEventsForce2() {
		double currentTime = CloudSim.clock();
		double minTime = Double.MAX_VALUE;
		double timeDiff = currentTime - getLastProcessTime();
		double timeFrameDatacenterEnergy = 0.0;
