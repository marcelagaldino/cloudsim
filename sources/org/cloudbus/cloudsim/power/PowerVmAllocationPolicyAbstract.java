...
public abstract class PowerVmAllocationPolicyAbstract extends VmAllocationPolicy {
	...
	public boolean allocateHostForVm(Vm vm, Host host) {
		...
		if (host.vmCreate(vm)) { // if vm has been succesfully created in the host
			...
			Log.formatLine(
					...
					CloudSim.clock());
			//vmsToHost(vm.getId(), host.getId(),CloudSim.clock());
			
			if (CloudSim.clock() == 0) {
				host.setInitialStatus(true);
			}
	...
	public void vmsToHost(int vm, int host, double clock){
		
		FileWriter arq = null;
		
		try {
		
			arq = new FileWriter("vmsToHost_experimento_preliminar.csv", true);
		...
		gravarArq.printf(clock+","+vm+","+host+"%n");
