...
public class Cloudlet {
	...
	public void setCloudletStatus(final int newStatus) throws Exception {
		...
		if (newStatus == Cloudlet.SUCCESS) {
			...
			double time_execution = finishTime - this.getSubmissionTime();
			
			
			
			fileTimeExecution(PlanetLabConstants.OV,time_execution,this.getCloudletId());
	 ...
	 public void fileTimeExecution(int ov, double timeExecution, int cloudletID ){
			
			FileWriter arq = null;
			
			try {
			
				arq = new FileWriter("te.csv", true);
			...
			gravarArq.printf(this.getExpName()+","+ov+","+timeExecution+","+cloudletID+","+this.getSubmissionTime()+","+this.getInputPath()+","+this.cloudletLength+","+"estatico"+"%n");
	...
	public double getUtilizationOfCpu2(final double time) {
		return getUtilizationModelCpu().getUtilization2(time);
	}
